...
#########################################################################
# NOV 15, XC:  note:  I revised 5 places: see Line 30, 38, 47, 57 and 139.
#########################################################################
...
train.roc.data = data.frame(GLM=(predict(model.glm,train.data,type="response") %>% data.frame())[,1],
                            ...
                            DT=as.numeric(predict(model.dt,train.data[,-1],type="prob")[,2]), # XC: we need to use type="prob"  (revise 1)
                            RF=predict(model.rf,train.data[,-1], type='prob')[,2],            # XC: we need to use type="prob"  (revise 1)
...
test1.roc.data = data.frame(GLM=(predict(model.glm,test1.data,type="response") %>% data.frame())[,1],
                            ...
                            DT=as.numeric(predict(model.dt,test1.data[,-1],type="prob")[,2]), # XC: we need to use type="prob" (revise 2)
                            RF=predict(model.rf,test1.data[,-1],type="prob")[,2],             # XC: we need to use type="prob" (revise 2)
...
test2.roc.data = data.frame(GLM=(predict(model.glm,test2.data,type="response") %>% data.frame())[,1],
                            ...
                            DT=as.numeric(predict(model.dt,test2.data[,-1],type="prob")[,2]),   # XC: we need to use type="prob" (revise 3)
                            RF=predict(model.rf,test2.data[,-1],type="prob")[,2],               # XC: we need to use type="prob" (revise 3)
...
### XC: delete this block #### (revise 4)
if(F){
train.roc_data$DT<-ifelse(train.roc_data$DT==2,1,0)
test1.roc_data$DT<-ifelse(test1.roc_data$DT==2,1,0)
test2.roc_data$DT<-ifelse(test2.roc_data$DT==2,1,0)
...
for (i in 1:length(model_name)) {
  print(i)
  roc_temp = roc(train.roc_data$FPFL ~ train.roc_data[,i+1])
  train_auc[i,"AUC"] = auc(roc_temp)
}
train_auc[,"Predictor"] = model_name
model_x_train_legend.name = paste(model_name,"AUC",round(as.numeric(train_auc$AUC),3),sep=" ")
...
model_x_test1_legend.name = paste(model_name,"AUC",round(as.numeric(test1_auc$AUC),3),sep=" ")
...
model_x_test2_legend.name = paste(model_name,"AUC",round(as.numeric(test2_auc$AUC),3),sep=" ")
...
g.list.test2 <- pROC::ggroc(roc.list.test2, legacy.axes = TRUE,aes=c("linetype", "color")) + 
  ...
  xlab("1-Specificity") + ylab("Sensitivity") +
  theme(legend.position = "right",legend.title=element_blank())+     #XC: remove 'name' (revise 5)
...
figure_file=paste0(getwd(),"/Figure/","Figure S1 model_21_lab.pdf")
dir.create(file.path(dirname(figure_file)))
ggsave(filename=figure_file,width = 16.2,height = 10)
