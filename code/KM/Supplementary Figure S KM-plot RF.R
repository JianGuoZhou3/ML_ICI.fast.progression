...
#B in RF
{
  ...
  # RF
  Figure_3a = survfit(Surv(OS.time, OS.CNSR) ~ group_RF, data = train.roc.sur) %>% 
    ggsurvplot(train.roc.sur, 
               ...
               palette = "nejm")
  res_cox = coxph(Surv(OS.time, OS.CNSR) ~ group_RF, data = train.roc.sur)
  ...
  # RF
  Figure_3c = survfit(Surv(OS.time, OS.CNSR) ~ group_RF, data = test1.roc.sur) %>% 
  ...
  res_cox = coxph(Surv(OS.time, OS.CNSR) ~ group_RF, data = test1.roc.sur)
  HR = paste("HR",round(summary(res_cox)$conf.int[1],2))
  CI = paste("(","95%CI:",round(summary(res_cox)$conf.int[3],2),"-",round(summary(res_cox)$conf.int[4],2),")",sep = "")
  P = paste("P",ifelse(round(summary(res_cox)$coef[5],4)==0,"< 0.0001",round(summary(res_cox)$coef[5],4)))
  annotext = paste(HR,CI)
  ...
  # RF
  Figure_3e = survfit(Surv(OS.time, OS.CNSR) ~ group_RF, data = test2.roc.sur) %>% 
  ...
  res_cox = coxph(Surv(OS.time, OS.CNSR) ~ group_RF, data = test2.roc.sur)
  HR = paste("HR",round(summary(res_cox)$conf.int[1],2))
  CI = paste("(","95%CI:",round(summary(res_cox)$conf.int[3],2),"-",round(summary(res_cox)$conf.int[4],2),")",sep = "")
  P = paste("P",ifelse(round(summary(res_cox)$coef[5],4)==0,"< 0.0001",round(summary(res_cox)$coef[5],4)))
  annotext = paste(HR,CI)
  ...
  # RF
  Figure_3b = survfit(Surv(PFS.time, PFSINV.CNSR) ~ group_RF, data = train.roc.sur) %>% 
    ggsurvplot(train.roc.sur, 
               legend.title = "", legend = c(0.8,0.99), legend.labs = c("predicted non-FP", "predicted FP"),# 图例标题/位置/标签
  ...
  res_cox = coxph(Surv(PFS.time, PFSINV.CNSR) ~ group_RF, data = train.roc.sur)
  HR = paste("HR",round(summary(res_cox)$conf.int[1],2))
  CI = paste("(","95%CI:",round(summary(res_cox)$conf.int[3],2),"-",round(summary(res_cox)$conf.int[4],2),")",sep = "")
  P = paste("P",ifelse(round(summary(res_cox)$coef[5],4)==0,"< 0.0001",round(summary(res_cox)$coef[5],4)))
  annotext = paste(HR,CI)
  ...
  # RF
  Figure_3d = survfit(Surv(PFS.time, PFSINV.CNSR) ~ group_RF, data = test1.roc.sur) %>% 
    ggsurvplot(test1.roc.sur, 
               break.time.by = 3,
               legend.title = "", legend = c(0.8,0.99), legend.labs = c("predicted non-FP", "predicted FP"),# 图例标题/位置/标签
               ylab = "Progression-Free survival (%)", xlab = "Time (Months)",title ="",surv.scale="percent", 
  ...
  res_cox = coxph(Surv(PFS.time, PFSINV.CNSR) ~ group_RF, data = test1.roc.sur)
  HR = paste("HR",round(summary(res_cox)$conf.int[1],2))
  CI = paste("(","95%CI:",round(summary(res_cox)$conf.int[3],2),"-",round(summary(res_cox)$conf.int[4],2),")",sep = "")
  P = paste("P",ifelse(round(summary(res_cox)$coef[5],4)==0,"< 0.0001",round(summary(res_cox)$coef[5],4)))
  annotext = paste(HR,CI)
  ...
  # RF
  Figure_3f = survfit(Surv(PFS.time, PFSINV.CNSR) ~ group_RF, data = test2.roc.sur) %>% 
    ggsurvplot(test2.roc.sur, 
               legend.title = "", legend = c(0.8,0.99), legend.labs = c("predicted non-FP", "predicted FP"),# 图例标题/位置/标签
               ylab = "Progression-Free survival (%)", xlab = "Time (Months)",title ="",surv.scale="percent", 
               surv.median.line = "hv", surv.plot.height = 0.6,
  ...
  res_cox = coxph(Surv(PFS.time, PFSINV.CNSR) ~ group_RF, data = test2.roc.sur)
  HR = paste("HR",round(summary(res_cox)$conf.int[1],2))
  CI = paste("(","95%CI:",round(summary(res_cox)$conf.int[3],2),"-",round(summary(res_cox)$conf.int[4],2),")",sep = "")
  P = paste("P",ifelse(round(summary(res_cox)$coef[5],4)==0,"< 0.0001",round(summary(res_cox)$coef[5],4)))
  annotext = paste(HR,CI)
  ...
  # Figure_3g-----
  Figure_3g = survfit(Surv(OS.time, OS.CNSR) ~ group_RF, data = all.roc.sur) %>% 
    ggsurvplot(test2.roc.sur, 
               legend.title = "", legend = c(0.8,0.99), legend.labs = c("predicted non-FP", "predicted FP"),# 图例标题/位置/标签
               ylab = "Overall survival (%)", xlab = "Time (Months)",title ="",surv.scale="percent", 
               surv.median.line = "hv", surv.plot.height = 0.6,
  ...
  res_cox = coxph(Surv(OS.time, OS.CNSR) ~ group_RF, data = all.roc.sur)
  HR = paste("HR",round(summary(res_cox)$conf.int[1],2))
  CI = paste("(","95%CI:",round(summary(res_cox)$conf.int[3],2),"-",round(summary(res_cox)$conf.int[4],2),")",sep = "")
  P = paste("P",ifelse(round(summary(res_cox)$coef[5],4)==0,"< 0.0001",round(summary(res_cox)$coef[5],4)))
  annotext = paste(HR,CI)
  ...
  # Figure_3h-----
  Figure_3h = survfit(Surv(PFS.time, PFSINV.CNSR) ~ group_RF, data = all.roc.sur) %>% 
    ggsurvplot(test2.roc.sur, 
               legend.title = "", legend = c(0.8,0.99), legend.labs = c("predicted non-FP", "predicted FP"),# 图例标题/位置/标签
               ylab = "Progression-Free survival (%)", xlab = "Time (Months)",title ="",surv.scale="percent", 
               surv.median.line = "hv", surv.plot.height = 0.6,
  ...
  res_cox = coxph(Surv(PFS.time, PFSINV.CNSR) ~ group_RF, data = all.roc.sur)
  HR = paste("HR",round(summary(res_cox)$conf.int[1],2))
  CI = paste("(","95%CI:",round(summary(res_cox)$conf.int[3],2),"-",round(summary(res_cox)$conf.int[4],2),")",sep = "")
  P = paste("P",ifelse(round(summary(res_cox)$coef[5],4)==0,"< 0.0001",round(summary(res_cox)$coef[5],4)))
  annotext = paste(HR,CI)
...
# C patch -----
{
  ...
  ggsave("Figure_S_KM_RF.pdf",width = 25.45,height = 12.5)
} #4*2