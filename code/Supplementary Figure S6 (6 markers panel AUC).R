...
# compare with 21 models
# choose one and compare with 21 markers panel
...

{
  ...
  # AUC/accuracy/PPV/recall/F1/Kapaa (new)----
  train_tables = data.frame() 
  for (i in 1:length(model_name)) {
    print(i)
    temp_roc = roc(train.roc_data$FPFL ~ train.roc_data[,i+1], levels = c(0,1),direction='<')
    cp = pROC::coords(temp_roc, 
                      "best", ret=c("threshold", "specificity", "sensitivity", "accuracy",
                                    "ppv", "npv","precision","recall"), transpose = FALSE)
    
    train_tables[i,1:8] = cp
    train_tables = train_tables %>% mutate(F1 = ppv * recall * 2 / (ppv + recall))
    
    train_tables[i,"AUC"] = auc(temp_roc)
    train_tables[i,"95% CI"] = paste0(round((ci.auc(temp_roc,conf.level=0.95, method=c("bootstrap"),boot.n=1000))[1],4),"-",
                                      round(ci.auc(temp_roc,conf.level=0.95, method=c("bootstrap"),boot.n=1000)[3],4))
    
    train_tables[i,"kappa"] = (vcd::Kappa(table(data.frame(true = train.roc_data$FPFL,pred = ifelse(train.roc_data[,i+1] > cp$threshold,1,0)))
                                          ,weights = "Equal-Spacing"))$Weighted[1]
    train_tables[i,"kappa CI"] = paste0(round((vcd::Kappa(table(data.frame(true = train.roc_data$FPFL,pred = ifelse(train.roc_data[,i+1] > cp$threshold,1,0)))
                                                          ,weights = "Equal-Spacing") %>% confint())[1,1],4),"-",
                                        round((vcd::Kappa(table(data.frame(true = train.roc_data$FPFL,pred = ifelse(train.roc_data[,i+1] > cp$threshold,1,0)))
                                                          ,weights = "Equal-Spacing") %>% confint())[1,2],4))
    
  }
  rownames(train_tables) = model_name
  
  test1_tables = data.frame() 
  for (i in 1:length(model_name)) {
    print(i)
    temp_roc = roc(test1.roc_data$FPFL ~ test1.roc_data[,i+1], levels = c(0,1),direction='<')
    cp = pROC::coords(temp_roc, 
                      "best", ret=c("threshold", "specificity", "sensitivity", "accuracy",
                                    "ppv", "npv","precision","recall"), transpose = FALSE)
    
    test1_tables[i,1:8] = cp
    test1_tables = test1_tables %>% mutate(F1 = ppv * recall * 2 / (ppv + recall))
    
    test1_tables[i,"AUC"] = auc(temp_roc)
    test1_tables[i,"95% CI"] = paste0(round((ci.auc(temp_roc,conf.level=0.95, method=c("bootstrap"),boot.n=1000))[1],4),"-",
                                      round(ci.auc(temp_roc,conf.level=0.95, method=c("bootstrap"),boot.n=1000)[3],4))
    
    test1_tables[i,"kappa"] = (vcd::Kappa(table(data.frame(true = test1.roc_data$FPFL,pred = ifelse(test1.roc_data[,i+1] > cp$threshold,1,0)))
                                          ,weights = "Equal-Spacing"))$Weighted[1]
    test1_tables[i,"kappa CI"] = paste0(round((vcd::Kappa(table(data.frame(true = test1.roc_data$FPFL,pred = ifelse(test1.roc_data[,i+1] > cp$threshold,1,0)))
                                                          ,weights = "Equal-Spacing") %>% confint())[1,1],4),"-",
                                        round((vcd::Kappa(table(data.frame(true = test1.roc_data$FPFL,pred = ifelse(test1.roc_data[,i+1] > cp$threshold,1,0)))
                                                          ,weights = "Equal-Spacing") %>% confint())[1,2],4))
    
  }
  rownames(test1_tables) = model_name
  
  test2_tables = data.frame() 
  for (i in 1:length(model_name)) {
    print(i)
    temp_roc = roc(test2.roc_data$FPFL ~ test2.roc_data[,i+1], levels = c(0,1),direction='<')
    cp = pROC::coords(temp_roc, 
                      "best", ret=c("threshold", "specificity", "sensitivity", "accuracy",
                                    "ppv", "npv","precision","recall"), transpose = FALSE)
    
    test2_tables[i,1:8] = cp
    test2_tables = test2_tables %>% mutate(F1 = ppv * recall * 2 / (ppv + recall))
    
    test2_tables[i,"AUC"] = auc(temp_roc)
    test2_tables[i,"95% CI"] = paste0(round((ci.auc(temp_roc,conf.level=0.95, method=c("bootstrap"),boot.n=1000))[1],4),"-",
                                      round(ci.auc(temp_roc,conf.level=0.95, method=c("bootstrap"),boot.n=1000)[3],4))
    
    test2_tables[i,"kappa"] = (vcd::Kappa(table(data.frame(true = test2.roc_data$FPFL,pred = ifelse(test2.roc_data[,i+1] > cp$threshold,1,0)))
                                          ,weights = "Equal-Spacing"))$Weighted[1]
    test2_tables[i,"kappa CI"] = paste0(round((vcd::Kappa(table(data.frame(true = test2.roc_data$FPFL,pred = ifelse(test2.roc_data[,i+1] > cp$threshold,1,0)))
                                                          ,weights = "Equal-Spacing") %>% confint())[1,1],4),"-",
                                        round((vcd::Kappa(table(data.frame(true = test2.roc_data$FPFL,pred = ifelse(test2.roc_data[,i+1] > cp$threshold,1,0)))
                                                          ,weights = "Equal-Spacing") %>% confint())[1,2],4))
    
  }
  rownames(test2_tables) = model_name
} # Calculate AUC
# save-----
up/down + plot_annotation(tag_levels = c('A', '1'))
ggsave("Figure S6 6 markers model.pdf",width = 16.2,height = 10)


# compare with 21 bio marker models----


Barplot_data_t = Barplot_data
View(compare_means(AUC ~ Predictor, data = Barplot_data_t))
...
Barplot_data$group = "21"
Barplot_data_t$group = "opt"


my_data = rbind(Barplot_data,Barplot_data_t)
colnames(my_data)
